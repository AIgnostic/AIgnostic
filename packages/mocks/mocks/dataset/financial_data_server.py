...
DATA_PATH = os.path.join(os.path.dirname(__file__), "FinancialPhraseBank-v1.0", "FinancialPhraseBank-v1.0", "Sentences_50Agree.txt")
...
async def fetch_datapoints(num_datapoints: int = Query(2, alias="n")):
    ...
    if num_datapoints > dataset_size:
        raise HTTPException(status_code=400, detail="Requested more data points than available in the dataset.")
