...
def generate_report(metrics_data: dict, api_key: str) -> list[dict]:
    ...
    Args:
        metrics_data (dict): A dictionary containing metrics data where keys are metric names and values are metric information.
        api_key (str): The API key required to initialize the language model (LLM).

    Returns:
        list[dict]: A list of dictionaries, each containing the report for a specific property. Each dictionary includes:
            - "property" (str): The name of the property.
            - "computed_metrics" (list[dict]): A list of dictionaries with metric names and their corresponding information.
