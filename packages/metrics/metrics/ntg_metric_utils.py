...
def generate_synonym_perturbations(sentence: str, mask='[MASK]') -> list[str]:
    """
...
def ntg_lime(info: CalculateRequest) -> tuple[np.array, Ridge]:
    """
    ...
    print("reached past shape input features")
    ...
    perturbed_samples = generate_masked_sequences(info.input_features, num_masked_per_sentence=ceil(average_sentence_length / 4))
    num_perturbations_per_sample = perturbed_samples.shape[1]
    perturbed_samples = np.array(perturbed_samples).reshape(-1).reshape(-1, 1)  # Flatten the array and reshape to 2D (convert from (N, 10) to (N*10, 1))
    
    # TODO: Consider batching inputs to the model API
    response: ModelResponse = _query_model(perturbed_samples, info)

    outputs = response.confidence_scores
    ...
    unperturbed_scores = info.confidence_scores

    # Call model endpoint to get confidence scores
    
    # Compute similarity weights using an RBF kernel
    diffs = []
    for i, c_score in enumerate(unperturbed_scores):
        for j in range(num_perturbations_per_sample):
            diffs.append(outputs[i * num_perturbations_per_sample + j] - c_score)

    # Choose l1 norm for numerical stability in large dimensions when comparing probabilities (low values)
    distances = np.linalg.norm(np.array(diffs), axis=1, ord=1) 
    ...
    reg_model.fit(np.array(diffs), outputs, sample_weight=weights)
