...
def test_high_stability_ntg(apply_server_factory):
    ...
    hs_info = CalculateRequest(
        ...
        model_api_key="None",
        # Update metrics library to only require confidence score based on metric, task_name combination
...
def test_low_stability_ntg(apply_server_factory):
    ...
    assert ls_result.metric_values[metric_name].computed_value < 0.3, ls_result