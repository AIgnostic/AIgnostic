        ...
        <View key={index} style={styles.section}>
          ...
          {Object.entries(section.computed_metrics).length > 0 ? (
                ...
                <View key={idx} style={{ marginBottom: 10 }}>
                  ...
                  {metric_info.error ? (
                    ...
                    <MetricBarPDF
                      ...
                      min={
                        metric_info.range[0] === null
                          ? -Infinity
                          : parseFloat(metric_info.range[0])
                      }
                      max={
                        metric_info.range[1] === null
                          ? Infinity
                          : parseFloat(metric_info.range[1])
                      }
                    />
                  )}
                </View>
            ...
            <View>
              <Text style={styles.subsection}>Computed Metrics</Text>
              <Text style={styles.text}>
                No metrics were computed for this property.
              </Text>
            </View>
          )}
                  ...
                  {leg.map((extract: LegislationExtract, idx1: number) => (
                    <View key={idx1}>
                      <Text style={[styles.text, { fontStyle: 'italic' }]}>
                        Article {extract.article_number}:{' '}
                        {extract.article_title}
                      </Text>
                      <Text style={styles.bulletPoint}>
                        • Link:{' '}
                        <Link src={extract.link} style={styles.hyperlink}>
                          {extract.link}
                        </Link>
                      </Text>
                      {}
