...
async def test_fetch_data_invalid_data_format_gives_worker_exception(mock_get):
    ...
    mock_response = MagicMock()
    mock_response.json.return_value = {"invalid": "data"}  # Not matching DatasetResponse schema
    mock_response.raise_for_status.return_value = None
    mock_get.return_value = mock_response
    ...
    assert "Data error - data returned from data provider of incorrect format" in str(excinfo.value)
...
async def test_query_model_invalid_data_format_gives_worker_exception(mock_post):
    """Test that WorkerException is raised when the data format is incorrect."""
    mock_response = MagicMock()
    mock_response.json.return_value = {"invalid": "data"}  # Not matching ModelResponse schema
    mock_response.raise_for_status.return_value = None
    mock_post.return_value = mock_response
