...
def test_server_validates_client_dataset_correctly_given_valid_url(start_mock_server):
    response = server_mock.get("/validate_dataset?url=" + valid_url)
    assert response.status_code == 200
...
def test_server_returns_400_error_given_unexpected_data_format(start_mock_server):
    response = server_mock.get("/validate_dataset?url=" + unexpected_data_url)
    assert response.status_code == 400
    assert response.json() == {"detail": "Invalid data format"}


def test_server_returns_400_error_on_invalid_url(start_mock_server):
    response = server_mock.get("/validate_dataset?url=" + invalid_url)
    assert response.status_code == 400
